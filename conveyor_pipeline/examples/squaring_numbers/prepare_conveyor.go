...
func PrepareComplexTransactionalConveyor(cnv *conveyor.Conveyor) (*conveyor.Conveyor, error) {
	...
	sub := &SubtractionOperator{
		ConcreteNodeExecutor: &conveyor.ConcreteNodeExecutor{Name: "number_subtractor"},
		ToSub:                4,
	}

	if err := cnv.AddNodeExecutor(sub, conveyor.WorkerModeTransaction, conveyor.WorkerTypeOperation); err != nil {
		fmt.Printf("Failed to add SubtractionOperator to conveyor :[+%v]", err)
	}

	/*
		Create a replication joint, and add it to conveyor using "AddJointExecutorAfterNode" method
	*/
	joint, jntErr := conveyor.NewReplicateJoint("replicator", 1)
	if jntErr != nil {
	...
	*/
	...
	if err := cnv.AddNodeExecutorAfterJoint(prnt4, conveyor.WorkerModeTransaction, conveyor.WorkerTypeSink); err != nil {
		fmt.Printf("Failed to add PrinterSink4 to conveyor :[+%v]", err)
	}


	//joint1, jntErr1 := conveyor.NewDoubleJoint("replicator", 1)
	//if jntErr != nil {
	//	fmt.Printf("Failed to add SquareOperator to conveyor :[+%v]", jntErr1)
	//	return nil, jntErr1
	//}
	//
	//if err := cnv.AddJointExecutorAfterNode(joint1, conveyor.WorkerModeLoop, conveyor.WorkerTypeJoint); err != nil {
	//	return nil, err
	//}
